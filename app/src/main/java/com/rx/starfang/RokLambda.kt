...
class RokLambda {
    ...
    //private val REGEX_DIGITS = "[0-9]"
    //private val REGEX_EXCEPT_DIGITS = "[^0-9]"
    //private val REGEX_SPACE = "\\s+"

    companion object {
        val debugTag = "rok_lambda"

        val alias = HashMap<String, String>( mapOf(
            "기마병" to "기병"
        ))

        val cmdKor3HashMap = HashMap<String, CmdNum>( mapOf(
            "사령관" to CmdNum.CMDR,
            "박물관" to CmdNum.RELIC
        ))


        val cmdKor2HashMap = HashMap<String, CmdNum>( mapOf(
            "장군" to CmdNum.CMDR,
            "스킬" to CmdNum.SKILL,
            "특성" to CmdNum.ATTR,
            "문명" to CmdNum.CIV,
        ...
        val cmdEngHashMap = HashMap<String, CmdNum>( mapOf(
            "cmdr" to CmdNum.CMDR,
    ...
    private fun findCmd(req: String?): Pair<CmdNum?,LangNum?> {

        var lang: LangNum = LangNum.KOR

        for (certain in com.starfang.nlp.lambda.LambdaRok.CMD_ENUM.values()) {
            try {
                val certainIndex = certain.ordinal
                if (certainIndex < CMD_CERTAIN.size && req != null) {
                    val probKey = CMD_CERTAIN[certainIndex]
                    val reqLength = req.length
                    val keyLength = probKey.length
                    if (reqLength >= keyLength) {
                        try {
                            if (req.substring(reqLength - keyLength) == probKey) {
                                cmd = certain
                                req = if (reqLength == keyLength) null else req.substring(
                                    0,
                                    reqLength - keyLength
                                ).trim { it <= ' ' }
